---
title: Unlocking The Power Of Asynchronous Actions In The EUTXO Model
description: 'ErgoForum discussion: Unlocking The Power Of Asynchronous Actions In
  The EUTXO Model'
tags:
- bootstrapping
- box
...
source: ergoforum
source_url: https://ergoforum.org/t/unlocking-the-power-of-asynchronous-actions-in-the-eutxo-model/502
created: '2020-12-09'
last_activity: '2020-12-10'
posts_count: 3
views: 1161
likes: 14
glossary_hits:
...
# Unlocking The Power Of Asynchronous Actions In The EUTXO Model

> **Forum thread:** [https://ergoforum.org/t/unlocking-the-power-of-asynchronous-actions-in-the-eutxo-model/502](https://ergoforum.org/t/unlocking-the-power-of-asynchronous-actions-in-the-eutxo-model/502)
> **Category:** Uncategorized | **Created:** 2020-12-09 | **Posts:** 3 | **Views:** 1161

**Related concepts:** Bootstrapping, Box, ErgoScript context, Invariant, Native tokens, State machine, Token ID rule, eUTXO

---

### Post #1 — @robert (2020-12-09)

This is an informal post that I wanted to get down before the idea leaves my head, so please bare with the quick writing/releasing of thoughts in a somewhat incoherent manner here.
...
### Post #2 — @kushti (2020-12-10)

![](https://yyz2.discourse-cdn.com/flex030/user_avatar/www.ergoforum.org/robert/48/8_2.png) robert:

> The bootstrap action that the user goes through forces him/her to mint i64 total new tokens (of a single id) into a UTXO that is locked under the “User Async Emission” contract. This contract only allows the user to emit tokens when they use other boxes from the same protocol as data-inputs together with their own boxes as inputs.

So a user both minting a token for tracking protocol progress and then making the progress? Sounds interesting!

---

### Post #3 — @robert (2020-12-10)

Precisely. And because the async tokens were minted during the bootstrap action and only emitted during async actions, whatever data comes out of these async actions is already pre-validated as correct without requiring reference to protocol state checkpoints or any other synchronizing action.
